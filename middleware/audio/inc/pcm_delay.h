 ...
 * @param high_quality High quality switch. If high_quality is set 1, the delay values are losslessly saved for high quality output, otherwise (high_quality = 0) the delay values are compressed for lower memory requirement.
 ...
 *        If high_quality is set 1, the buffer capacity = "max_delay_samples*2*num_channels" in bytes for 16 bit PCM and "max_delay_samples*4*num_channels" in bytes for 24 bit PCM. 
 *           For example if PCM is 16 bit, max_delay_samples = 22050, then the buffer capacity should be 88200 bytes (22050*2*2) for stereo input or 44100 bytes (22050*2*1) for mono input.
 *           For example if PCM is 24 bit, max_delay_samples = 22050, then the buffer capacity should be 176400 bytes (22050*4*2) for stereo input or 88200 bytes (22050*4*1) for mono input.
 ...
 *           the required buffer capacities are the same for both 16 bit and 24 bit PCM.
...
/**
 * @brief Apply PCM delay to a frame of PCM data (24-bit).
 * @param ct Pointer to a PCMDelay object.
 * @param pcm_in Address of the PCM input. The data layout for mono: M0,M1,M2,...; for stereo: L0,R0,L1,R1,L2,R2,...
 * @param pcm_out Address of the PCM output. The data layout for mono: M0,M1,M2,...; for stereo: L0,R0,L1,R1,L2,R2,...
 *        pcm_out can be the same as pcm_in. In this case, the PCM signals are changed in-place.
 ...
 * @return error code. PCMDELAY_ERROR_OK means successful, other codes indicate error.
 */
int32_t pcm_delay_apply24(PCMDelay* ct, int32_t *pcm_in, int32_t *pcm_out, int32_t n, int32_t delay_samples);
